...
class CreateDepartmentDataTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('department_saved_data', function (Blueprint $table) {
            ...
            $table->integer('last_year_offer')->nullable()->comment('去年聯合分發錄取名額');
            $table->integer('last_year_amount')->nullable()->comment('去年聯合分發名額（只有學士班有聯合分發）');
            $table->integer('amount')->nullable()->comment('聯合分發名額（只有學士班有聯合分發）');
            $table->string('decrease_reason')->nullable()->comment('減招原因');
            ...
            $table->integer('rank')->nullable()->default(99999)->comment('志願排名');
