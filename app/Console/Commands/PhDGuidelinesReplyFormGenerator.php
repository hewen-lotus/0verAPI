...
class PhDGuidelinesReplyFormGenerator extends Command
{
    ...
    public function handle()
    {
        if (SchoolHistoryData::where('id', '=', $this->argument('school_code'))
            ...
            $all_depts_id = $data->graduate_departments()->select('id')->distinct()->where('system_id', '=', 4)->get();
            ...
            $table .= '<tr><th>總計</th><td colspan="4">' . $total_dept . ' 系組 / (個人申請：' . $total_admission_selection_quota . ' 人，自招；' . (int)$system->ratify_quota_for_self_enrollment . ' 人)<br />上學年度新生總量 10%：' . (int)$system->last_year_admission_amount . ' 人,本國學生碩士班未招足名額：' . (int)$system->last_year_surplus_admission_quota . ' 人, 教育部核定擴增名額：' . (int)$system->ratify_expanded_quota . ' 人</td></tr>';
            ...
            foreach ($depts as $dept) {
                ...
                $table .= '<td rowspan="2" style="width: 15%; text-align: center; vertical-align: middle">' . $dept->id . '</td>';
