...
class PhDGuidelinesReplyFormGenerator extends Command
{
    ...
    public function handle()
    {
        if (SchoolHistoryData::where('id', '=', $this->argument('school_code'))
            ...
            $depts = $data->graduate_departments()->where('system_id', '=', 4)->get();
            ...
            $system = $data->systems()->where('type_id', '=', 4)->get();

            $table .= '<tr><th>總計</th><td colspan="4">' . $data->graduate_departments()->where('system_id', '=', 4)->count() . ' 系組 / (個人申請：' . $total_admission_selection_quota . ' 人，自招；' . $total_self_enrollment_quota . ' 人)<br />上學年度新生總量 10%：' . (int)$system['0']['last_year_admission_amount'] . ' 人,本國學生碩士班未招足名額：' . (int)$system['0']['last_year_surplus_admission_quota'] . ' 人, 教育部核定擴增名額：' . (int)$system['0']['ratify_expanded_quota'] . ' 人</td></tr>';
            ...
            if ($this->argument('email')) {
                $mpdf->Output(sys_get_temp_dir() . '/' . $data->title . '-博士班簡章調查回覆表.pdf', 'F');
                ...
                Mail::send('emails.guidelines-reply-form', [], function ($m) use ($data) {
                    ...
                    $m->attach(sys_get_temp_dir() . '/' . $data->title . '-博士班簡章調查回覆表.pdf');
                });

                $this->info('信件已寄出！');
            } else {
                $mpdf->Output(storage_path('app/public/' . $data->title . '-博士班簡章調查回覆表.pdf'), 'F');
