...
class EngPhDGuidelinesReplyFormGenerator extends Command
{
     ...
     * @var string
     */
    protected $signature = 'pdf-generator:eng-phd-guidelines-reply-form
     ...
     * @var string
     */
    protected $description = '系統輸出英文版博士班簡章調查回覆表';
    ...
    public function handle()
    {
        if (SchoolHistoryData::where('id', '=', $this->argument('school_code'))
            ...
            $all_depts_id = $data->graduate_departments()->select('id')->distinct()->where('system_id', '=', 4)->orderBy('sort_order', 'asc')->get();
            ...
            foreach ($all_depts_id as $all_dept_id) {
                $dept = $data->graduate_departments()->where('system_id', '=', 4)->where('id', '=', $all_dept_id->id)->latest()->first();
