...
class EngMasterGuidelinesReplyFormGenerator extends Command
{
     ...
     * @var string
     */
    protected $signature = 'pdf-generator:eng-master-guidelines-reply-form
     ...
     * @var string
     */
    protected $description = '系統輸出英文版碩士班簡章調查回覆表';
    ...
    public function handle()
    {
        if (SchoolHistoryData::where('id', '=', $this->argument('school_code'))
            ...
            $all_depts_id = $data->graduate_departments()->select('id')->distinct()->where('system_id', '=', 3)->orderBy('sort_order', 'asc')->get();
            ...
            foreach ($all_depts_id as $all_dept_id) {
                $dept = $data->graduate_departments()->where('system_id', '=', 3)->where('id', '=', $all_dept_id->id)->latest()->first();
            ...
            foreach ($depts as $dept) {
                ...
                $table .= '<tr><td style="width: 44%;">' . $dept->eng_description . '</td><td style="width: 44%;">' . $doc_output . '</td></tr></table>';
            }
