    ...
    public function handle()
    {
        if (SchoolHistoryData::where('id', '=', $this->argument('school_code'))
            ...
            if ($this->argument('email')) {
                $mpdf->Output(sys_get_temp_dir() . '/' . $data->title . '-碩士班簡章調查回覆表.pdf', 'F');
                ...
                Mail::send('emails.guidelines-reply-form', [], function ($m) use ($data) {
                    ...
                    $m->attach(sys_get_temp_dir() . '/' . $data->title . '-碩士班簡章調查回覆表.pdf');
                });

                $this->info('信件已寄出！');
            } else {
                $mpdf->Output(storage_path('app/public/' . $data->title . '-碩士班簡章調查回覆表.pdf'), 'F');
